...
const categoryInfo = async (req, res) => {
  try {
    ...
    const categoryData = await Category.find({}).sort({ addedDate: -1 }).skip(skip).limit(limit)
...
const addCategory = async (req, res) => {
  try {
    ...
    if (description.trim().length < 20) {
      return res.status(400).json({
        success: false,
        message: `Description must be at least 20 characters (currently ${description.trim().length})`,
      })
    }
...
const updateProductsWithCategoryOffer = async (categoryId, offerValue, maxRedeemable = 0) => {
  try {
    ...
    console.log(`Updated ${products.length} products with category offer: ${offerValue}%`)
  } catch (error) {
