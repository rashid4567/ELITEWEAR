  ...
  try {
    ...
    const formattedProductData = productData.map((product) => {
      ...
      const totalStock = product.variants.reduce(
        (sum, variant) => sum + (variant.varientquatity || 0),
        0
      );
      return {
        ...product.toObject(),
    ...
    if (variants) {
      if (Array.isArray(variants)) {
        parsedVariants = variants.map((variant) => ({
          ...
          salePrice:
            parseInt(variant.varientPrice || 0) *
        ...
        if (Object.keys(variants).some((key) => !isNaN(parseInt(key)))) {
          ...
          parsedVariants.push({
            ...
            salePrice:
              parseInt(variants.varientPrice || 0) *
              (1 - parseInt(productOffer || 0) / 100),
        ...
        while (req.body[`variants[${index}][size]`]) {
          parsedVariants.push({
            ...
            varientPrice:
              parseInt(req.body[`variants[${index}][varientPrice]`]) || 0,
            salePrice:
              parseInt(req.body[`variants[${index}][varientPrice]`] || 0) *
              (1 - parseInt(productOffer || 0) / 100),
            varientquatity:
              parseInt(req.body[`variants[${index}][varientquatity]`]) || 0,
          });
          index++;
        }
      }
...
const editProduct = async (req, res) => {
  try {
    ...
    let removeImagesArray = Array.isArray(removeImages)
      ? removeImages
      : removeImages
      ? [removeImages]
      : [];
      ...
      const indexToRemove = images.findIndex(
        (img) =>
          ...
          (fieldName !== "mainImage" &&
            ...
            fieldName ===
              `additionalImage${
                images.filter((i) => !i.isMain).indexOf(img) + 1
              }`)
      );
      if (indexToRemove !== -1) {
        const imageToRemove = images[indexToRemove];
        const publicId =
          imageToRemove.public_id || `products/${id}-${fieldName}`;
...
const deleteProduct = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(productId)) {
      return res
        .status(400)
        .json({ status: false, message: "Invalid product ID format" });
    }
