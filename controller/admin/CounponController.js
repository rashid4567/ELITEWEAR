...
const editCoupon = async (req, res) => {
  try {
    ...
    if (
      !updateData.coupencode ||
      !updateData.couponpercent ||
      ...
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: coupencode, couponpercent, startingDate, and expiryDate are required",
      });
    }

    if (updateData.coupencode) {
      const existingCoupon = await Coupon.findOne({
        coupencode: updateData.coupencode,
        _id: { $ne: id },
      });
      if (existingCoupon) {
        return res
