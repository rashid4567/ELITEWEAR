...
const createWalletTopupOrder = async (req, res) => {
  try {
    ...
    logger.info(`Creating wallet top-up order for user: ${userId}, Amount: ${amount}`)
...
const verifyWalletTopupPayment = async (req, res) => {
  try {
    ...
    if (expectedSignature !== razorpay_signature) {
      logger.error(`Invalid signature for wallet top-up: ${razorpay_payment_id}`)
      return res.status(400).json({
        success: false,
    ...
    if (!topupDetails || topupDetails.orderId !== razorpay_order_id) {
      logger.error(`Invalid or missing session order details: ${JSON.stringify(topupDetails)}`)
      return res.status(400).json({
        success: false,
    ...
    logger.info(`User wallet balance updated from ${previousBalance} to ${user.walletBalance}`)
