...
const { generateOrderNumber, generateTransactionId } = require("../../utils/helpers")
...
const getRazorpayInstance = () => {
  if (!process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_KEY_SECRET) {
    ...
    throw new Error("Razorpay credentials missing. Please check environment variables.")
  }
...
const createRazorpayOrder = async (req, res) => {
  try {
    ...
    console.log("Creating Razorpay order for user:", userId, "Coupon:", couponCode)
    ...
    for (const item of cart.items) {
      ...
      if (!variant) {
        console.error("Invalid variant for product:", item.productId.name, "Size:", item.size)
    ...
    // Create cartItems array in the same format as placeOrder function
    ...
    if (couponCode) {
      ...
      const now = new Date()
      if (now < new Date(coupon.startingDate) || now > new Date(coupon.expiryDate)) {
      ...
      if (userUsage && userUsage.usageCount >= coupon.limit) {
        console.error("Coupon usage limit reached for user:", userId, "Coupon:", couponCode)
        return res.status(400).json({
          success: false,
      ...
      // Use the same proportional discount calculation as in placeOrder
    ...
    if (!addressId) {
      ...
      return res.status(400).json({ success: false, message: "Delivery address not selected" })
    }
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        retryCount++
        console.error(`Razorpay API error (attempt ${retryCount}):`, JSON.stringify(razorpayError, null, 2))
        if (retryCount === maxRetries) {
          return res.status(500).json({
            success: false,
            message: `Razorpay error: ${razorpayError.error?.description || "Failed to create order"}`,
    ...
    // Create pending order with detailed discount information
    ...
    // Store detailed discount information in session
...
const verifyPayment = async (req, res) => {
  try {
      ...
      if (pendingOrderId) {
        ...
        if (order) {
          ...
          if (orderDetails.couponCode) {
            ...
            if (coupon) {
              const userIndex = coupon.usedBy.findIndex((usage) => usage.userId.toString() === userId.toString())
              if (userIndex !== -1) {
                coupon.usedBy[userIndex].usageCount += 1
                // Check if orders array exists, if not initialize it
                if (!coupon.usedBy[userIndex].orders) {
                  coupon.usedBy[userIndex].orders = []
                }
                coupon.usedBy[userIndex].orders.push(order._id)
              } else {
                coupon.usedBy.push({
                  userId,
                  usageCount: 1,
    ...
    if (orderDetails.couponCode) {
      ...
      if (coupon) {
        ...
        if (userIndex !== -1) {
          ...
          coupon.usedBy[userIndex].orders.push(pendingOrder._id)
        } else {
          coupon.usedBy.push({
            userId,
            usageCount: 1,
    ...
    console.log("Payment verified and order processed successfully:", pendingOrder._id)
...
const retryRazorpayPayment = async (req, res) => {
  try {
    ...
    if (
      ...
      (failedOrder.paymentStatus !== "Failed" && failedOrder.paymentStatus !== "Pending")
    ) {
      return res.status(400).json({
        success: false,
    ...
    const orderItems = await OrderItem.find({ orderId: failedOrder._id })
    ...
    return res.status(200).json({
      ...
      discountInfo: {
        items: itemsWithDiscount,
        totalDiscount: failedOrder.discount,
        finalTotal: failedOrder.total - failedOrder.deliveryCharge,
        grandTotal: failedOrder.total,
      },
    })
  } catch (error) {
